...
class PyTorchEfficientNetV2TrainingTool(Tool):

    name = "pytorch_efficientnetv2_training"
    description = """
    This tool trains an EfficientNetV2 model using PyTorch for medical image classification.
    It can train from scratch or fine-tune a pre-trained model, and includes validation metrics.
    Supports EfficientNetV2-S, EfficientNetV2-M, and EfficientNetV2-L variants with configurable augmentation.
    ...
    inputs = {
        ...
        "model_variant": {
            "type": "string",
            "description": "EfficientNetV2 variant: efficientnet_v2_s, efficientnet_v2_m, or efficientnet_v2_l",
            "required": False,
            "nullable": True
        },
        "num_epochs": {
        ...
        "use_custom_image_size": {
            "type": "boolean",
            "description": "Whether to use a custom image size instead of the default for the variant",
            "required": False,
            "nullable": True
        },
        "custom_image_size": {
            "type": "integer",
            "description": "Custom image size to use (applies to both width and height)",
            "required": False,
            "nullable": True
        },
        "augmentation_level": {
    ...
    def forward(
        ...
        output_dir: str,
        num_classes: int,
        model_variant: Optional[str] = "efficientnet_v2_s",
        num_epochs: Optional[int] = 10,
        batch_size: Optional[int] = 16,
        pretrained: Optional[bool] = True,
        early_stopping: Optional[bool] = True,
        patience: Optional[int] = 5,
        use_custom_image_size: Optional[bool] = False,
        custom_image_size: Optional[int] = 224,
        ...
        """
        Train an EfficientNetV2 model for medical image classification with class imbalance handling.

        Args:
            data_dir: Directory containing dataset with train and val subdirectories
            output_dir: Directory to save model and results
            num_classes: Number of classes for classification
            model_variant: EfficientNetV2 variant (efficientnet_v2_s, efficientnet_v2_m, efficientnet_v2_l)
            num_epochs: Number of training epochs
            batch_size: Batch size for training
            pretrained: Whether to use pretrained weights
            early_stopping: Whether to use early stopping
            patience: Number of epochs to wait before early stopping
            use_custom_image_size: Whether to use a custom image size
            custom_image_size: Custom image size to use
        ...
        """
        try:
            # Define default image sizes for each EfficientNetV2 variant
            default_image_sizes = {
            ...
            if use_custom_image_size:
                image_size = custom_image_size
                logging.info(f"Using custom image size: {image_size}x{image_size}")
            else:
                image_size = default_image_sizes.get(model_variant, 384)
                logging.info(f"Using default image size for {model_variant}: {image_size}x{image_size}")

            efficientnet_image_size = (image_size, image_size)
            ...
            # Log configuration
            logging.info(f"Starting EfficientNetV2 training with configuration:")
            ...
            # Define augmentation transforms (keeping the original implementation)
            ...
            train_transform = get_augmentation_transforms(augmentation_level, efficientnet_image_size, custom_augmentations)
            ...
            if handle_class_imbalance and train_class_dist is not None:
                ...
                if imbalance_strategy in ['oversampling', 'undersampling']:
                    # We need to extract features for sampling
                    # For image data, we'll use image paths and apply sampling to indices
                    ...
                    # Create a simplified feature representation (using image paths as identifiers)
            ...
            if model_variant not in efficientnet_models:
                logging.warning(f"Invalid model variant: {model_variant}. Using efficientnet_v2_s instead.")
                model_variant = 'efficientnet_v2_s'

            model_fn, weights = efficientnet_models[model_variant]
