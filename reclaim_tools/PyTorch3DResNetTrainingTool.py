...
class PyTorch3DResNetTrainingTool(Tool):
    """Training tool for 3D ResNet models using MONAI for medical image classification."""

    name = "pytorch_3dresnet_training"
    description = """
    This tool trains a 3D ResNet model using PyTorch and MONAI for 3D medical image classification.
    Designed for MedMNIST datasets (28x28x28 volumes) but supports any 3D medical imaging data.
    Supports ResNet10, ResNet18, ResNet34, and ResNet50 variants with configurable 3D augmentation.
    ...
    def forward(
        ...
        output_dir: str,
        num_classes: int,
        model_variant: Optional[str] = "resnet18",
        ...
        """
        Train a 3D ResNet model for medical image classification.
        ...
        try:
            ...
            logging.info(f"Starting 3D ResNet training with configuration:")
            ...
            # MONAI ResNet parameters: spatial_dims, n_input_channels, num_classes
            # When using pretrained weights, feed_forward must be False
            model_kwargs = {
                'spatial_dims': 3,
                'n_input_channels': 1,
            ...
            # If using pretrained weights, replace the final fully connected layer
            # to match the target number of classes (pretrained model has 2048 classes)
            if pretrained:
                # MONAI ResNet uses 'fc' as the final layer name
                if hasattr(model, 'fc'):
                    if model.fc is not None:
                        in_features = model.fc.in_features
                    else:
                        # When feed_forward=False, fc is None, determine in_features from architecture
                        # For ResNet50, after layer4 and avgpool, the feature dimension is 2048
                        if model_variant == 'resnet50':
                            in_features = 2048
                        elif model_variant == 'resnet34' or model_variant == 'resnet18':
                            in_features = 512
                    ...
                    logging.info(f"Replaced final FC layer: {in_features} -> {num_classes} classes")
                    ...
                    logging.info(f"Replaced final classifier layer: {in_features} -> {num_classes} classes")
