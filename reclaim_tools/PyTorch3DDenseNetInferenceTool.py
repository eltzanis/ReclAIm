...
class PyTorch3DDenseNetInferenceTool(Tool):
    """Inference tool for 3D DenseNet models using MONAI."""

    name = "pytorch_3ddensenet_inference"
    description = """
    This tool uses a trained PyTorch 3D DenseNet model to perform inference on 3D medical images.
    Supports both standard and custom DenseNet configurations (auto-detected from model config).
    ...
    inputs = {
        ...
        "config_type": {
            "type": "string",
            "description": "Configuration type: 'custom' or 'standard' (auto-detected from config file if available)",
            "required": False,
            "nullable": True
        }
    }
    ...
    def forward(
        ...
        """
        Run inference using a trained 3D DenseNet model on 3D medical images.
        ...
        try:
            ...
            if config_path and os.path.exists(config_path):
                ...
                try:
                    ...
                    logging.info(f"Model configuration loaded")
                except Exception as e:
                    logging.warning(f"Error loading model configuration: {str(e)}")

            # Set parameters
            if model_variant is None:
                model_variant = model_config.get('model_variant', 'densenet121')

            if config_type is None:
                config_type = model_config.get('config_type', 'custom')
            ...
            logging.info(f"Starting 3D DenseNet inference:")
            ...
            # MONAI DenseNet - recreate architecture matching the training configuration
            model = DenseNet(
                spatial_dims=3,
                in_channels=1,
